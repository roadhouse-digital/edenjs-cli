  ...
  async render(path, options, callback) {
    ...
    try {
      ...
      await eden.hook('view.head', head, () => {
        ...
        head += `<link rel="stylesheet" href="${config.get('cdn.url') || '/'}public/css/app.min.css${config.get('app.version') ? `?v=${config.get('app.version')}` : ''}" data-eden="head-start" id="eden-prehead">`;
        head += render.page.head || '';
        head += '<meta name="eden" value="head-end" data-eden="head-end" id="eden-posthead">';
        head += '<meta name="author" value="rd">';
      });
      ...
      await eden.hook('view.foot', ({
        ...
        // Add to foot
        foot += '<!-- USER.END -->';
        foot += `<script data-eden="script" id="eden-script" type="text/javascript" src="${config.get('cdn.url') || '/'}public/js/app.min.js${config.get('app.version') ? `?v=${config.get('app.version')}` : ''}"></script>`;
